...
proc get_ooc_results { implementations } {
   ...
   foreach ooc $implementations {
      ...
      set residual_xdc   [get_attribute ooc $ooc residual_xdc]
      ...
      foreach xdc_file $residual_xdc {
         puts "\tRunning xdc file $xdc_file for $hierInst"
         command "source $xdc_file" "$resultDir/residual_xdc.log"
      }
...
proc generate_pr_bitstreams { configs } {
   ...
   foreach config $configs {
      ...
      set bitstream.pre [get_attribute config $config bitstream.pre]
      ...
      if {$bitstream} {
         ...
         set end_time [clock seconds]
         log_time write_bitstream $start_time $end_time 1 "[llength $configs] Configurations"
